<?php
	...
	class AutoImg
	{
		...
		public function __construct() {
			$this->documentRoot = dirname(dirname(dirname(__FILE__))) . DS;
			
			$this->query = $_GET['p'];
			
			$this->cacheFile = dirname(__FILE__) . DS . $this->cacheDir . DS . md5($this->query);
		...
		private function parseParams() {
			...
			if(substr($this->imageFile, 0, 4) != 'http') {
				$this->imageFile = $this->documentRoot . DS . $this->imageFile;
			}
